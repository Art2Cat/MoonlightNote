    ...
    companion object {
        ...
        fun saveUserToCache(context: Context?, user: User?) {
            ...
            if (!user?.uid.isNullOrEmpty()) {
                SPUtils.putString(context, "User", "Id", user?.uid)
            } else {
            ...
            if (!user?.email.isNullOrEmpty()) {
                SPUtils.putString(context, "User", "Email", user?.email)
            } else {
                SPUtils.putString(context, "User", "Email", null)
            }

            if (!user?.photoUrl.isNullOrEmpty()) {
                SPUtils.putString(context, "User", "PhotoUrl", user?.photoUrl)
            } else {
                SPUtils.putString(context, "User", "PhotoUrl", null)
            }

            if (!user?.token.isNullOrEmpty()) {
                SPUtils.putString(context, "User", "Token", user?.token)
            } else {
                SPUtils.putString(context, "User", "Token", null)
            }

            if (!user?.encryptKey.isNullOrEmpty()) {
                SPUtils.putString(context, "User", "EncryptKey", user?.encryptKey)
            } else {
        ...
        fun getUserFromCache(context: Context): User {
            val user = User()
            user.uid = SPUtils.getString(context, "User", "Id", null)
            user.nickname = SPUtils.getString(context, "User", "Username", null)
            user.email = SPUtils.getString(context, "User", "Email", null)
            user.photoUrl = SPUtils.getString(context, "User", "PhotoUrl", null)
            user.token = SPUtils.getString(context, "User", "Token", null)
            user.encryptKey = SPUtils.getString(context, "User", "EncryptKey", null)
            return user
        }
